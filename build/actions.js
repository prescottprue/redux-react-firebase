...
var login = exports.login = function login(dispatch, firebase, credentials) {
  return new _es6Promise.Promise(function (resolve, reject) {
    ...
    if (provider) {
      ...
      var auth = type === 'popup' ? firebase.auth().authWithOAuthPopup : firebase.auth().authWithOAuthRedirect;
...
var resetPassword = exports.resetPassword = function resetPassword(dispatch, firebase, credentials) {
  dispatchLoginError(dispatch, null);
  firebase.auth().resetPassword(credentials, function (err) {
    if (err) {
      switch (err.code) {
        case 'INVALID_USER':
...
var changePassword = exports.changePassword = function changePassword(dispatch, firebase, credentials) {
  dispatchLoginError(dispatch, null);
  firebase.auth().changePassword(credentials, function (err) {
    if (err) {
      switch (err.code) {
...
exports.default = { watchEvents: watchEvents, unWatchEvents: unWatchEvents, init: init, logout: logout, createUser: createUser, resetPassword: resetPassword, changePassword: changePassword };